 ...
 * {@linkplain FnExpr function calls} or {@link NewInstanceExpr new instances}.
 */
class ObjExpr implements Expr {
	...
	void compile(String superName, String[] interfaceNames) throws IOException {
		...
		if(supportsMeta())
			{
			cv.visitField(ACC_FINAL, "__meta", Compiler.IPERSISTENTMAP_TYPE.getDescriptor(), null, null);
			}
		...
		for(int i=0;i<protocolCallsites.count();i++)
			{
			cv.visitField(ACC_PRIVATE + ACC_STATIC, cachedClassName(i), Compiler.CLASS_TYPE.getDescriptor(), null, null);
			}
		...
		if(supportsMeta())
			{
			ctorgen.loadThis();
			ctorgen.visitVarInsn(Compiler.IPERSISTENTMAP_TYPE.getOpcode(ILOAD), 1);
		...
		for(ISeq s = RT.keys(closes); s != null; s = s.next(), ++a)
			...
			if(primc != null)
				{
				ctorgen.visitVarInsn(Type.getType(primc).getOpcode(ILOAD), a);
				...
				{
				ctorgen.visitVarInsn(Compiler.OBJECT_TYPE.getOpcode(ILOAD), a);
		...
		if(supportsMeta())
			...
			ctorgen.visitCode();
			ctorgen.loadThis();
			ctorgen.visitInsn(ACONST_NULL);	//null meta
			ctorgen.loadArgs();
			ctorgen.invokeConstructor(objtype, new Method("<init>", Type.VOID_TYPE, ctorTypes));
...
//		getCompiledClass();
		if(isDeftype())
			...
			for(ISeq s = RT.seq(closesExprs); s != null; s = s.next())
				{
                Compiler.LocalBindingExpr lbe = (Compiler.LocalBindingExpr) s.first();
	...
	public Class getJavaClass() {
		return (compiledClass != null) ? compiledClass
			: (tag != null) ? Compiler.HostExpr.tagToClass(tag)
			: IFn.class;
	}
	...
	public void emitAssignLocal(GeneratorAdapter gen, Compiler.LocalBinding lb,Expr val){
		...
		if(primc != null)
			...
			{
			val.emit(EvaluationContext.EXPRESSION, this, gen);
			gen.putField(objtype, lb.name, Compiler.OBJECT_TYPE);
			}
	}

	void emitLocal(GeneratorAdapter gen, Compiler.LocalBinding lb, boolean clear){
		if(closes.containsKey(lb))
			...
			if(primc != null)
				{
				gen.getField(objtype, lb.name, Type.getType(primc));
				Compiler.HostExpr.emitBoxReturn(this, gen, primc);
				}
			else
				{
				gen.getField(objtype, lb.name, Compiler.OBJECT_TYPE);
			...
			if(lb.isArg)
				...
				if(primc != null)
					{
					gen.visitVarInsn(Type.getType(primc).getOpcode(ILOAD), lb.idx);
					Compiler.HostExpr.emitBoxReturn(this, gen, primc);
					}
				else
                    {
	...
	void emitUnboxedLocal(GeneratorAdapter gen, Compiler.LocalBinding lb){
		int argoff = isStatic?0:1;
		Class primc = lb.getPrimitiveType();
		if(closes.containsKey(lb))
			{
