...
class NewInstanceExpr extends ObjExpr {
	...
	static class DeftypeParser implements IParser {
		public Expr parse(EvaluationContext context, final Object frm) {
			...
			ObjExpr ret = build((IPersistentVector)RT.get(opts, Compiler.implementsKey,PersistentVector.EMPTY),fields,null,tagname, classname,
                rform, frm);
			return ret;
		}
	}
	...
	public Expr parse(EvaluationContext context, Object frm) {
		...
		ObjExpr ret = build(interfaces, null, null, classname, Symbol.intern(classname), rform, frm);
		if(frm instanceof IObj && ((IObj) frm).meta() != null)
			return new Compiler.MetaExpr(ret, Compiler.MapExpr
...
//		ret.volatiles = PersistentHashSet.create(RT.seq(RT.get(ret.optionsMap, volatileKey)));
		...
		try
			...
			for(ISeq s = methodForms; s != null; s = RT.next(s))
				{
				Compiler.NewInstanceMethod m = Compiler.NewInstanceMethod.parse(ret, (ISeq) RT.first(s), thistag, overrideables);
	...
	static Class compileStub(String superName, NewInstanceExpr ret, String[] interfaceNames, Object frm){
		...
		ClassVisitor cv = cw;
		cv.visit(V1_5, ACC_PUBLIC + ACC_SUPER, Compiler.COMPILE_STUB_PREFIX + "/" + ret.internalName,
		...
		if(this.isDeftype())
			...
			if (this.isDeftype() && this.fields.count() > this.hintedFields.count())
				...
				for(ISeq s = RT.seq(hintedFields); s!=null; s=s.next(), i++)
					...
					if(k.isPrimitive())
						{
						mv.visitTypeInsn(CHECKCAST, Type.getType(Compiler.boxClass(k)).getInternalName());
						}
				...
				clojure.asm.commons.Method ctor = new clojure.asm.commons.Method("<init>", Type.VOID_TYPE, ctorTypes());
	...
	static public Map[] gatherMethods(Class sc, ISeq interfaces){
		...
		Map<IPersistentVector,Method> mm = new HashMap<IPersistentVector,Method>();
